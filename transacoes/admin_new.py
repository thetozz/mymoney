...
from .models import (
    Categoria, ContaBancaria, Transacao, TransacaoRecorrente, ImportacaoOFX
)
...
class TransacaoRecorrenteAdmin(admin.ModelAdmin):
    list_display = [
        'descricao', 'valor', 'tipo', 'tipo_recorrencia',
    ...
    fieldsets = (
        ...
        ('Configurações de Recorrência', {
            'fields': (
                'tipo_recorrencia', 'dia_vencimento', 'data_inicio',
                'data_fim', 'ativa'
            )
        }),
        ('Auditoria', {
...
class ImportacaoOFXAdmin(admin.ModelAdmin):
    list_display = [
        'arquivo_nome', 'data_importacao', 'usuario',
