...
class TransacaoRecorrente(models.Model):
    ...
    tipo_recorrencia = models.CharField(
        max_length=11, choices=TIPOS_RECORRENCIA, default='MENSAL')
    dia_vencimento = models.IntegerField(
        default=1, help_text='Dia do mês (1-31)')
    data_inicio = models.DateField(default=timezone.now)
    data_fim = models.DateField(
        null=True, blank=True, help_text='Deixe vazio para recorrência indefinida')
    ativa = models.BooleanField(default=True)
    ...
    def proxima_data_vencimento(self, data_referencia=None):
        ...
        if self.tipo_recorrencia == 'MENSAL':
            proxima_data = data_referencia.replace(
        ...
        ultimo_dia_mes = calendar.monthrange(
            proxima_data.year, proxima_data.month)[1]
